...
export const MISSIONS: Mission[] = [
  /* --------------------- Chapter 1 — Async JavaScript --------------------- */
  {
    ...
    objectives: [
      { text: "Reproduce the blocked event loop", done: false },
      { text: "Identify the synchronous hot path", done: false },
      { text: "Move the work off the main thread", done: false },
      { text: "Verify latency returns to normal", done: false },
    ],
    ...
    briefing: {
      ...
      steps: [
        "Reproduce the slowdown across the API, not just the reporting endpoint.",
        "Identify what is holding the runtime using logs, metrics, code and the trace.",
        "Apply the best fix and verify the event loop recovers.",
      ],
      skills: ["Event Loop", "Node.js Runtime", "Root-Cause Analysis"],
      context: [
        { label: "Affected endpoint", value: "GET /api/reports/weekly" },
        { label: "First reported", value: "8 min after the v3.8.0 deploy" },
        { label: "Impact", value: "Every endpoint slows down; requests time out" },
  ...
  },
  {
    ...
    objectives: [
      { text: "Trace the rejected promise", done: false },
      { text: "Compare allSettled vs all", done: false },
      { text: "Add partial-failure handling", done: false },
      { text: "Confirm the batch survives failures", done: false },
    ],
    ...
    briefing: {
      ...
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce the failed enrichment run and read what it actually did.",
        "Work out why 47 successful vendor calls produced nothing.",
        "Apply the best fix and verify the run survives a failing vendor.",
      ],
      skills: ["Promises", "Error Handling", "Root-Cause Analysis"],
      context: [
        { label: "Affected job", value: "Nightly vendor enrichment" },
        {
          label: "First reported",
          value: "The morning after northwind started flapping",
        },
        { label: "Impact", value: "0 of 48 vendors enriched for five nights" },
        { label: "Environment", value: "Node.js 20 · production" },
      ],
    },
  },
  {
    ...
    briefing: {
      ...
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce a batch that reports success in milliseconds.",
        "Work out what the job was waiting for before it declared success.",
        "Apply the best fix and verify every upload is processed.",
      ],
      skills: ["Async JavaScript", "Background Jobs", "Root-Cause Analysis"],
      context: [
        { label: "Affected job", value: "process-uploads (media-worker)" },
        { label: "First reported", value: "A week after release 1.9.0" },
        {
          label: "Impact",
          value: "187 of 500 uploads silently missing thumbnails",
        },
        { label: "Environment", value: "Node.js 20 · production" },
      ],
    },
  },
  {
    ...
    briefing: {
      ...
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce a window where the same invoice is charged twice.",
        "Establish where the second run came from before blaming the provider.",
        "Apply the best fix and verify runs can no longer overlap.",
      ],
      skills: ["Background Jobs", "Async JavaScript", "Log Analysis"],
      context: [
        { label: "Affected job", value: "billing-sync invoice charge" },
        {
          label: "First reported",
          value: "Six weeks after the pricing migration",
        },
        { label: "Impact", value: "38 customers double-charged in 24 hours" },
        { label: "Environment", value: "Node.js 20 · production" },
      ],
    },
  },
  {
    ...
    briefing: {
      ...
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce a crash and read what Node says about why it happened.",
        "Establish whether the process is ending itself or being ended.",
        "Apply the best fix and verify both uptime and message delivery.",
      ],
      skills: ["Error Handling", "Node.js Runtime", "Process Lifecycle"],
      context: [
        { label: "Affected service", value: "notification-service v2.4.1" },
        { label: "First reported", value: "Within an hour of the 2.4.1 release" },
        {
          label: "Impact",
          value: "41 restarts an hour; 214 messages lost per crash",
        },
        { label: "Environment", value: "Node.js 20 · production" },
      ],
    },
  ...
  },
  {
    ...
    objectives: [
      { text: "Reproduce a forced logout", done: true },
      { text: "Follow every refresh attempt in one expiry", done: false },
      { text: "Diagnose why a valid session is revoked", done: false },
      { text: "Fix the client and verify sessions survive", done: false },
    ],
    rewardSkill: "Authentication +1",
    rewardSkillId: "authentication",
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Auth", "Backend"],
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce a logout by loading a page that fires several API calls at once.",
        "Count the refresh attempts and check which token each one presents.",
        "Apply the best fix and verify sessions survive an expiry.",
      ],
      skills: ["Authentication", "Async JavaScript", "Root-Cause Analysis"],
      context: [
        { label: "Affected endpoint", value: "POST /auth/refresh" },
        { label: "First reported", value: "After the 6.1.0 parallel-dashboard release" },
        { label: "Impact", value: "6.1% of sessions ended while still valid" },
        { label: "Environment", value: "Node.js 20 · production" },
      ],
    },
  },
  {
    ...
    objectives: [
      { text: "Review health-check timings", done: true },
      { text: "Find the dependency that decides the outcome", done: false },
      { text: "Diagnose why a healthy process is restarted", done: false },
      { text: "Ship the fix and confirm instances stay up", done: false },
    ],
    rewardSkill: "API Design +1",
    rewardSkillId: "api-design",
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Reliability", "API Design"],
      firstPhase: "Reproduce the Issue",
      steps: [
        "Watch an instance fail its probe while it is still answering requests.",
        "Break the health request down by dependency.",
        "Apply the best fix and verify capacity holds while a dependency is degraded.",
      ],
      skills: ["API Design", "Process Lifecycle", "Metrics Analysis"],
      context: [
        { label: "Affected endpoint", value: "GET /health (liveness + readiness)" },
        { label: "First reported", value: "During a third-party analytics slowdown" },
        { label: "Impact", value: "37 restarts in 30 minutes; 11.4% order errors" },
        { label: "Environment", value: "Node.js 20 · 8 instances · production" },
      ],
    },
  },
  {
    ...
    objectives: [
      { text: "Observe dropped requests during a deploy", done: true },
      { text: "Measure what happens between SIGTERM and exit", done: false },
      { text: "Diagnose what the process failed to wait for", done: false },
      { text: "Ship the fix and verify a clean rollout", done: false },
    ],
    rewardSkill: "Process Lifecycle +1",
    rewardSkillId: "process-lifecycle",
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Reliability", "Process Lifecycle"],
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce the errors by triggering a rollout.",
        "Measure the interval between the signal arriving and the process ending.",
        "Apply the best fix and verify requests, transactions and jobs all complete.",
      ],
      skills: ["Process Lifecycle", "API Design", "Background Jobs"],
      context: [
        { label: "Affected service", value: "checkout-api v5.3.1" },
        { label: "First reported", value: "Every deployment window" },
        { label: "Impact", value: "23 requests and 31 jobs disrupted per deploy" },
        { label: "Environment", value: "Node.js 20 · rolling deploys · production" },
      ],
    },
  },
  {
    ...
    objectives: [
      { text: "Reproduce the overshoot under load", done: true },
      { text: "Compare what each instance reads and writes", done: false },
      { text: "Diagnose why increments disappear", done: false },
      { text: "Ship the fix and load-test the limiter", done: false },
    ],
    rewardSkill: "API Design +1",
    rewardSkillId: "api-design",
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Concurrency", "API Design"],
      firstPhase: "Reproduce the Issue",
      steps: [
        "Reproduce the overshoot with concurrent traffic across several instances.",
        "Compare the counter value each instance reads with the value it writes.",
        "Apply the best fix and verify the limit holds at any replica count.",
      ],
      skills: ["API Design", "Async JavaScript", "Metrics Analysis"],
      context: [
        { label: "Affected endpoint", value: "All rate-limited public API routes" },
        { label: "First reported", value: "After scaling from 1 to 8 instances" },
        { label: "Impact", value: "147 requests allowed against a limit of 100" },
        { label: "Environment", value: "Node.js 20 · 8 instances · production" },
      ],
    },
  },
  ...
  /* ---------------- Chapter 3 — Workers and Performance ------------------ */
  {
    ...
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Worker Threads", "Backend"],
      firstPhase: "Measure the Growth",
      steps: [
        "Establish that memory is retained rather than in use.",
        "Find what still references jobs that already finished.",
        "Apply the best fix and confirm the heap returns to baseline.",
      ],
      skills: ["Closures and Memory", "EventEmitter", "Performance Debugging"],
      context: [
        { label: "Affected service", value: "image-worker pool (4 workers)" },
        { label: "First reported", value: "Seventh memory restart in 24h" },
        { label: "Impact", value: "Workers recycled every ~3h; jobs slow before each restart" },
        { label: "Environment", value: "Node.js 20 · 1.5GB memory limit · production" },
      ],
    },
  },
  {
    ...
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Background Jobs", "Backend"],
      firstPhase: "Measure the Drain Rate",
      steps: [
        "Compare how fast jobs arrive with how fast they actually complete.",
        "Find what the workers are spending their capacity on.",
        "Apply the best fix and confirm the backlog drains and stays down.",
      ],
      skills: ["Background Jobs", "Error Handling", "Metrics Analysis"],
      context: [
        { label: "Affected service", value: "notification-worker (24 workers)" },
        { label: "First reported", value: "Queue depth alert, 26 min ago" },
        { label: "Impact", value: "Notifications delayed 40+ min; scaling made it worse" },
        { label: "Environment", value: "Node.js 20 · external delivery provider · production" },
      ],
    },
  },
  {
    ...
    briefing: {
      severity: "high",
      technologies: ["Node.js", "Concurrency", "Backend"],
      firstPhase: "Separate Waiting From Working",
      steps: [
        "Establish where the request time is actually spent.",
        "Match connection checkouts against releases and find the gap.",
        "Apply the best fix and confirm the pool keeps headroom under load.",
      ],
      skills: ["Performance Debugging", "Error Handling", "Metrics Analysis"],
      context: [
        { label: "Affected endpoint", value: "GET /api/orders/:id" },
        { label: "First reported", value: "18 min into the morning traffic ramp" },
        { label: "Impact", value: "p95 4.9s, acquire timeouts, database idle" },
        { label: "Environment", value: "Node.js 20 · pool max 20 · production" },
      ],
    },
  },
  {
    ...
    briefing: {
      ...
      skills: ["Request Performance", "Performance Debugging", "Root-Cause Analysis"],
      context: [
...
export const SKILL_META: Record<string, { icon: LucideIcon; cls: string }> = {
  ...
  Promises: { icon: Zap, cls: "text-electric-300" },
  "Error Handling": { icon: Bug, cls: "text-rose-300" },
  "Process Lifecycle": { icon: Server, cls: "text-emerald-300" },
