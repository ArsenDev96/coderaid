...
export function generateMetadata({ params }: Params): Metadata {
  ...
  return {
    title: mission
      ? `${mission.title} — Verification | CodeRaid`
      : "Verification — CodeRaid",
  };
}

export default function VerificationPage({ params }: Params) {
  const mission = getMission(params.missionId);
  if (!mission) notFound();

  const verification = getVerification(mission.id);

  // Verification content is authored per mission — the before/after numbers only
  // mean something against that mission's fix, so they can't be derived.
  if (!verification) {
    const playable = VERIFIABLE_MISSION_IDS.map(getMission).filter(
      ...
      <DashboardShell active="Missions">
        <div className="mx-auto max-w-2xl py-10 text-center">
          ...
          <h1 className="mt-5 text-2xl font-bold tracking-tight text-white sm:text-3xl">
            Verification: {mission.title}
          </h1>
          <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-slate-400">
            The verification step for this mission is still being written — your
            progress is saved.
            {playable.length > 0 && (
              <>
                {" "}
                ...
                {playable.map((m, i) => (
                  <span key={m.id}>
                    ...
                    <Link
                      href={`/missions/${m.id}/verification`}
  ...
  const { step, totalSteps } = missionStep("Verification");
